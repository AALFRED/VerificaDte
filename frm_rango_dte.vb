...
Public Class frm_rango_dte
    ...
    Public stArch As String
    Public stArch2 As String
    ...
    Dim doc As New XmlTextWriter("C:\Proyectos Vb2017\verificadte\TokenCint\Token.xml", Encoding.UTF8)
    Dim docserializado = ("C:\Proyectos Vb2017\verificadte\TokenCint\Tokenserial.xml")
    Dim docxml = ("C:\Proyectos Vb2017\verificadte\TokenCint\Token.xml")
    ...
    Declare Function GetPrivateProfileString Lib "kernel32" Alias "GetPrivateProfileStringA" (ByVal _
        lpApplicationName As String, ByVal _
         lpKeyName As Byte, ByVal _
         lpDefault As String, ByVal _
         lpReturnedString As String, ByVal _
         nSize As Long, ByVal _
         lpFileName As String) As Long

    Private Function exportDataGridView(sfile As String, dgv As DataGridView) As Boolean
    ...
    Sub Crea_Log()
        ...
        Dim strDestinationFile As String = "C:\Proyectos Vb2017\verificadte\log\\" & Label25.Text & "-CINT-" & DatoHora & ".txt"
    ...
    Function GridAExcel(ByVal ElGrid As DataGridView) As Boolean
        ...
        Try
            ...
            exHoja.SaveAs("C:\Proyectos Vb2017\verificadte\Temp\Rep_error_Cint.xlsx")
            exLibro.Close()
            exApp.Quit()
    ...
    Sub busca_xml()
        ...
        Dim str As String = dt.ToString(format) 'transforma la primera fecha
        Dim str2 As String = dt2.ToString(format) 'transforma la segunda fecha
        ...
        Else
            ...
            stArch = ("\\192.168.1.250\SistemaERP\FAE_BSOFT02\02\dte\")
    ...
    Sub busca_pdf() 'facturas
        ...
        Select Case encuentra
            Case "FEL"
                If valor < "FEL0000095497" Then
    ...
    Sub busca_pdf2() 'notas de credito
        ...
        Select Case encuentra
            Case "FEL"
                If valor < "FEL0000095497" Then
                    stArch2 = ("\\192.168.1.250\SistemaERP\FAE_BSOFT02\02\pdf\")
                Else
                    stArch2 = ("\\192.168.1.250\EmpDte\CINTEGRAL\PDF\")
                End If
            Case "NCE"
                If valor < "NCE0000003962" Then
                    stArch2 = ("\\192.168.1.250\SistemaERP\FAE_BSOFT02\02\pdf\")
    ...
    Public Sub CreaXmlToken(ByVal seed As String)
        ...
        Try
            ...
            If doc.WriteState = WriteState.Closed Then
                ...
                Dim RutaDestinoRecibo As String = "C:\Proyectos Vb2017\verificadte\TokenCint\Token2.xml"
    ...
    Private Sub frm_rango_dte_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        frm_menu.Show()

    End Sub


    Private Sub frm_rango_dte_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Button2.Enabled = False 'consulta ndb
    ...
    Private Sub frm_rango_dte_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LostFocus
        frm_menu.Show()
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        'define la conexion a la base de datos
        'Dim conn As New SqlConnection("Server=190.151.5.59\SQL2008;Database=CINTEGRAL2017;User Id=sa;Password=Rs1399")
        'Dim conn As New SqlConnection("Server=192.168.1.28\SQL2008;Database=CINTEGRAL" & miano & ";User Id=sa;Password=Xtreme_1720")
    ...
    Private Sub Calendar1_DateSelected(ByVal sender As Object, ByVal e As System.Windows.Forms.DateRangeEventArgs) Handles Calendar1.DateSelected
        fecha1.Text = Calendar1.SelectionRange.Start.Date
        'fecha1.Text = Calendar1.SelectionRange.Start.ToString
        Calendar1.Visible = False

        'Date.TryParseExact(fecha1.Text, "dd-MM-yyyy", Globalization.CultureInfo.CurrentCulture, Globalization.DateTimeStyles.None, mifecha)
        'Date.TryParseExact(fecha1.Text, "dd-MM-yyyy", Globalization.CultureInfo.CurrentCulture, Globalization.DateTimeStyles.AssumeLocal, mifecha)
        mifecha = fecha1.Text
        fecha2.Focus()
    End Sub
    ...
    Private Sub cmd_buscar_NCR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_buscar_NCR.Click
        'BUSCA NCR
        'define la conexion a la base de datos
        'Dim conn As New SqlConnection("Server=190.151.5.59\SQL2008;Database=CINTEGRAL2016;User Id=sa;Password=Rs1399")
        'Dim conn As New SqlConnection("Server=192.168.1.28\SQL2008;Database=CINTEGRAL" & miano & ";User Id=sa;Password=Xtreme_1720")
        If ch_ano_ant.Checked = True Then
            miano = Date.Now.Year - 1
        Else
    ...
    Private Sub ch_va_correlativo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ch_va_correlativo.Click
        ' Dim fila As Integer

        'If ch_va_correlativo.Checked = True Then
        'Dim valor As Long
        'Dim i As Integer
        'valor = Val(Label15.Text)
        'With Me.grilla
        'For fila = valor To Label16.Text
        ...
        'If .Item(fila, 0) = valor Then
        ' 'MsgBox "Verdadero para  " & .TextMatrix(fila, 0)
        ' valor = valor + 1
        '.Row = fila
        '.Col = 0
        '.CellBackColor = &HC0FFC0
        '.CellForeColor = &H400000
        'Else
        'MsgBox("Se detectó una diferencia entre el Folio N°  " & .TextMatrix(fila - 1, 0) & " y el Folio N° " & .TextMatrix(fila, 0), vbCritical)
        '.Row = fila
        '.Col = 0
        '.CellBackColor = vbRed
        '.CellForeColor = vbWhite
        'If fila = .Rows - 1 Then
        ' valor = .TextMatrix(fila, 0)
        ' Else
        ' valor = .TextMatrix(fila + 1, 0)
        ' End If
        ' End If
        '         Next fila
        ' End With
    ...
    Private Sub ch_va_correlativo_CheckedChanged(sender As Object, e As EventArgs) Handles ch_va_correlativo.CheckedChanged
                ...
                For i = 1 To grilla.Rows.Count - 1
                    If Convert.ToInt32(grilla.Rows(i).Cells(0).Value.ToString()) > numero Then
                        ...
                        If numero = valor Then
                            MsgBox("Se finalizó de revisar el listado", MsgBoxStyle.Information + MessageBoxButtons.OK)
                        ...
                        Else
                            ...
                            Else
                                MsgBox("Se detectó un salto de folio en docto. nro " & numero & ", verifique!", MsgBoxStyle.Critical + MessageBoxButtons.OK)
                                .Rows(.CurrentRow.Index + 1).Selected = True
                            End If
                        End If
                    End If
    ...
    Private Sub grilla2_CellFormatting(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles grilla2.CellFormatting
        'Dim row As DataGridViewRow = grilla.Rows(e.RowIndex)
        'Dim row As DataGridViewRow = grilla.Rows(grilla.Rows.Count - 1)
        'Dim cell As DataGridViewCell = grilla.Rows(grilla.Rows.Count - 1).Cells(0)
    ...
    Private Sub cmd_exp_txt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_exp_txt.Click
        Try
            ...
            Else
                ...
                Dim fname As String = "C:\Proyectos Vb2017\verificadte\Temp\Exp_error_Cint.txt"
    ...
    Private Sub frm_rango_dte_Activated(sender As Object, e As EventArgs) Handles Me.Activated
